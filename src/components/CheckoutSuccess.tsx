...
const CheckoutSuccess = ({ onOrderSuccess, onCartClear }) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const txnId = params.get("transactionId") || params.get("txnId");
      const oid =
        params.get("orderId") ||
    ...
    const fetchStatus = async () => {
      try {
        ...
        if (!currentOrderId) {
          ...
          console.error("CheckoutSuccess - Missing order details:", {
            orderId,
            transactionId,
            sessionStorage: sessionStorage.getItem("pendingOrderId"),
          });
          return;
        }
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ fontSize: "12px", color: "#444", marginTop: "20px" }}>
            Verification Attempt: {pollCount}/{MAX_POLLS}
          </p>
        </div>
      </div>
    );
      ...
      >
        <div
          ...
          style={{
            ...
            borderRadius: "32px",
            textAlign: "center",
            border: `1px solid ${isSuccess ? "rgba(76,217,100,0.2)" : "rgba(255,59,48,0.2)"}`,
            position: "relative",
            overflow: "hidden",
          }}
        >
          ...
          <p
            style={{ color: "#888", marginBottom: "40px", fontSize: "1.1rem" }}
          >
            {isSuccess
              ? "Your tears are officially on the way. Get ready for the heat!"
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <p
                  style={{
                    fontWeight: "900",
                    fontSize: "1.2rem",
                    color: isSuccess ? "#4cd964" : "#fff",
                  }}
                >
                  {amount ? `₹${amount}` : "Calculating..."}
                </p>
              </div>
            </div>
                ...
                <>
                  <div style={{ display: "flex", gap: "15px" }}>
                    ...
                    <div>
                      ...
                      >
                        {customer.address ||
                          "Address details will be sent via email"}
                      </p>
                    </div>
                  </div>
                </>
